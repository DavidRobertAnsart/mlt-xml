...
const CONSUMER_ATTRIBUTES_SET = new Set<Partial<keyof Consumer>>(['id']);
export function consumerToXml(consumer: Consumer, depth?: string) {
  const propertiesSet = new Set(Object.keys(consumer).filter((key) => key !== 'id'));
  return objToXml(consumer, 'consumer', CONSUMER_ATTRIBUTES_SET, propertiesSet, [], depth);
}
